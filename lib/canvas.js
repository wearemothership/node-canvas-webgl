...
class NodeCanvasElement extends Canvas {
	...
	getContext(type, options) {
		...
		// Block WebGL 2 support as headless-gl doesn't support it
		if(type === "webgl2") {
			return null;
		}
		...
		if(type === "webgl") {
			...
			// Block WebGL 2.0 specific extensions and features
			const _getExtension = ctx.getExtension;
			ctx.getExtension = function(name) {
				// Block WebGL 2.0 specific extensions
				const webgl2Extensions = [
					"OES_vertex_array_object", // Available in WebGL 1 but commonly used differently in WebGL 2
					"WEBGL_draw_buffers",
					"EXT_color_buffer_float"
				];
        
				// Note: We're not blocking all WebGL 2 extensions since some have WebGL 1 equivalents
